...
globalValues_new <- function(kdeObs, kdeBench, obsData, benchData) {
  # Calculate the median value of Benchmark
  meanIntegral <- 0
  i <- 1
  while (meanIntegral < 0.5) {
...
# Prepare data for passing to the function
obsData <- data.frame(name = locations_industry$Name, lat = industry_lat, lon = industry_lon)
benchData <- data.frame(name = locations_bench$Name, lat = bench_lat, lon = bench_lon)
...
```{r}
concentrated_companies <- do.call(rbind, lapply(result$concentratedCompanies, as.data.frame))
write.csv(concentrated_companies, file = "concentrated_companies.csv", row.names = FALSE)
```
...
locations_industry <- read_excel("data/ind_com.xlsx")
locations_bench <- read_excel("data/bench_com.xlsx")
